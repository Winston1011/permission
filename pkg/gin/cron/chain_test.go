...
func TestChain(t *testing.T) {
	...
	NewChain(append1, append2, append3).Then(append4).Run(nil)
...
func TestChainRecover(t *testing.T) {
	panickingJob := FuncJob(func(ctx *gin.Context) error {
...
func (j *countJob) Run(ctx *gin.Context) error {
	j.m.Lock()
	j.started++
	j.m.Unlock()
	time.Sleep(j.delay)
...
func TestChainSkipIfStillRunning(t *testing.T) {
	...
	t.Run("different jobs independent", func(t *testing.T) {
		...
		for i := 0; i < 11; i++ {
			go wrappedJob1.Run(nil)
			go wrappedJob2.Run(nil)
		}
