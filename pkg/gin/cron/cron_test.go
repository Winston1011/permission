...
func TestFuncPanicRecovery(t *testing.T) {
	var buf syncWriter
	cron := New(gin.New(), WithParser(secondParser),
		WithChain(Recover()))
	cron.Start()
	defer cron.Stop()
	cron.AddFunc("* * * * * ?", func(ctx *gin.Context) error {
		panic("YOLO")
	})
...
func (d DummyJob) Run(ctx *gin.Context) error {
	panic("YOLO")
}
...
func TestStopCausesJobsToNotRun(t *testing.T) {
	...
	cron.Start()
	cron.Stop()
	cron.AddFunc("* * * * * ?", func(ctx *gin.Context) error { wg.Done(); return nil })
...
func TestAddWhileRunningWithDelay(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("* * * * * *", func(ctx *gin.Context) error { atomic.AddInt64(&calls, 1); return nil })
...
func TestRemoveBeforeRunning(t *testing.T) {
	...
	cron := newWithSeconds()
	id, _ := cron.addFunc("* * * * * ?", func(ctx *gin.Context) error { wg.Done(); return nil })
	cron.Remove(id)
	cron.Start()
	defer cron.Stop()
...
func TestSnapshotEntries(t *testing.T) {
	...
	cron := New(gin.New())
	cron.AddFunc("@every 2s", func(ctx *gin.Context) error { wg.Done(); return nil })
	cron.Start()
	defer cron.Stop()
...
func TestMultipleEntries(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc("0 0 0 1 1 ?", func(ctx *gin.Context) error { return nil })
	cron.AddFunc("* * * * * ?", func(ctx *gin.Context) error { wg.Done(); return nil })
	id1, _ := cron.addFunc("* * * * * ?", func(ctx *gin.Context) error { t.Fatal(); return nil })
	id2, _ := cron.addFunc("* * * * * ?", func(ctx *gin.Context) error { t.Fatal(); return nil })
	cron.AddFunc("0 0 0 31 12 ?", func(ctx *gin.Context) error { return nil })
	cron.AddFunc("* * * * * ?", func(ctx *gin.Context) error { wg.Done(); return nil })
...
func TestRunningMultipleSchedules(t *testing.T) {
	...
	cron.AddFunc("0 0 0 31 12 ?", func(ctx *gin.Context) error { return nil })
	cron.AddFunc("* * * * * ?", func(ctx *gin.Context) error { wg.Done(); return nil })
	cron.Schedule(Every(time.Minute), FuncJob(func(ctx *gin.Context) error { return nil }))
	cron.Schedule(Every(time.Second), FuncJob(func(ctx *gin.Context) error { wg.Done(); return nil }))
	cron.Schedule(Every(time.Hour), FuncJob(func(ctx *gin.Context) error { return nil }))
...
func TestLocalTimezone(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc(spec, func(ctx *gin.Context) error { wg.Done(); return nil })
	cron.Start()
	defer cron.Stop()
...
func TestNonLocalTimezone(t *testing.T) {
	...
	cron := New(gin.New(), WithLocation(loc), WithParser(secondParser))
	cron.AddFunc(spec, func(ctx *gin.Context) error { wg.Done(); return err })
	cron.Start()
	defer cron.Stop()
...
func (t testJob) Run(ctx *gin.Context) error {
	t.wg.Done()
	return nil
}
...
func TestScheduleAfterRemoval(t *testing.T) {
	...
	cron := newWithSeconds()
	hourJob := cron.Schedule(Every(time.Hour), FuncJob(func(ctx *gin.Context) error { return nil }))
	cron.Schedule(Every(time.Second), FuncJob(func(ctx *gin.Context) error {
		mu.Lock()
		defer mu.Unlock()
		switch calls {
		case 0:
...
func TestJobWithZeroTimeDoesNotRun(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("* * * * * *", func(ctx *gin.Context) error { atomic.AddInt64(&calls, 1); return nil })
	cron.Schedule(new(ZeroSchedule), FuncJob(func(ctx *gin.Context) error { t.Error("expected zero task will not run"); return nil }))
	cron.Start()
	defer cron.Stop()
	<-time.After(OneSecond)
	if atomic.LoadInt64(&calls) != 1 {
...
func TestStopAndWait(t *testing.T) {
	...
	t.Run("a couple fast jobs added, still returns immediately", func(t *testing.T) {
		cron := newWithSeconds()
		cron.AddFunc("* * * * * *", func(ctx *gin.Context) error { return nil })
		cron.Start()
		cron.AddFunc("* * * * * *", func(ctx *gin.Context) error { return nil })
		cron.AddFunc("* * * * * *", func(ctx *gin.Context) error { return nil })
		cron.AddFunc("* * * * * *", func(ctx *gin.Context) error { return nil })
	...
	t.Run("a couple fast jobs and a slow job added, waits for slow job", func(t *testing.T) {
		...
		cron.AddFunc("* * * * * *", func(ctx *gin.Context) error { return nil })
		cron.Start()
		cron.AddFunc("* * * * * *", func(ctx *gin.Context) error { time.Sleep(2 * time.Second); return nil })
		cron.AddFunc("* * * * * *", func(ctx *gin.Context) error { return nil })
		time.Sleep(time.Second)
...
func newWithSeconds() *Cron {
	return New(gin.New(), WithParser(secondParser), WithChain())
}
