...
func New(engine *gin.Engine, opts ...Option) *Cron {
	c := &Cron{
		entries:   nil,
		chain:     NewChain(),
		add:       make(chan *Entry),
...
type FuncJob func(ctx *gin.Context) error
...
func (c *Cron) AddFunc(spec string, cmd FuncJob) (err error) {
	_, err = c.AddJob(spec, cmd)
	return err
}

func (c *Cron) addFunc(spec string, cmd FuncJob) (id EntryID, err error) {
...
func (c *Cron) run() {
	...
	for _, entry := range c.entries {
		...
		// c.logger.Debug(nil, "schedule", "now", now, "entry", entry.ID, "next", entry.Next)
	}
	...
	for {
		...
		for {
			...
			case now = <-timer.C:
				...
				// c.logger.Debug(nil, "wake", "now", now)
			...
			case newEntry := <-c.add:
				...
				// c.logger.Debug(nil, "added", "now", now, "entry", newEntry.ID, "next", newEntry.Next)
			...
			case id := <-c.remove:
				...
				now = c.now()
				c.removeEntry(id)
				// c.logger.Debug("removed , entry: " + id.String())
			}
...
func (c *Cron) runWithRecovery(e *Entry) {
	...
	defer gin.RecycleContext(c.gin, ctx)

	ctx.CustomContext.Handle = e.Job.(FuncJob)
	ctx.CustomContext.Desc = e.Spec
	ctx.CustomContext.Type = "Cron"
