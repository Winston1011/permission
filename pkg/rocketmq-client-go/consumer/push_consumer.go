...
func (pc *pushConsumer) GetConsumerRunningInfo() *internal.ConsumerRunningInfo {
	...
	nsAddr := ""
	for _, value := range pc.client.Namesrvs().AddrList() {
...
func (pc *pushConsumer) pullMessage(request *PullRequest) {
	...
	NEXT:
		...
		case primitive.PullNoNewMsg:
			...
			pc.lastPullOffset.Store(request.mq, result.NextBeginOffset)
...
func (pc *pushConsumer) SendBack(msg *primitive.MessageExt) error {
	if pc.sendMessageBack("", msg, 0) {
		return nil
	}
	return fmt.Errorf("failed to send message back")
}

func (pc *pushConsumer) sendMessageBack(brokerName string, msg *primitive.MessageExt, delayLevel int) bool {
	var brokerAddr string
	if len(brokerName) != 0 {
		brokerAddr = pc.client.Namesrvs().FindBrokerAddrByName(brokerName)
	} else {
	...
	if err != nil {
		rlog.Error("failed to send message back", map[string]interface{}{
			"err":    err,
			"broker": brokerAddr,
		})
		return false
...
func (pc *pushConsumer) consumeMessageOrderly(pq *processQueue, mq *primitive.MessageQueue) {
	...
	if pc.model == BroadCasting || (pq.IsLock() && !pq.isLockExpired()) {
		...
		for continueConsume {
			...
			// this must happen before takeMessages
			pullOffset, pullOffsetExists := pc.lastPullOffset.Load(mq)
			...
			if len(msgs) == 0 {
				...
				// all messages consumed and no new ones, but some newer messges may be
				// filtered out, we should commit them to the broker
				currOffset := pc.storage.read(mq, _ReadMemoryThenStore)
				shouldUpdate := pullOffsetExists && currOffset < pullOffset.(int64) && !pq.IsDroppd()
				if shouldUpdate {
					pc.storage.update(mq, pullOffset.(int64), true)
				}
				rlog.Debug("no new messages found", map[string]interface{}{
					rlog.LogKeyMessageQueue: mq,
					"offset":                currOffset,
					"pull":                  pullOffset,
			...
			orderlyCtx.MQ = *mq
			orderlyCtx.SuspendCurrentQueueTime = pc.option.SuspendCurrentQueueTime
			...
			if pc.option.AutoCommit {
				...
				case SuspendCurrentQueueAMoment:
					if pc.checkReconsumeTimes(msgs) {
						...
						time.Sleep(orderlyCtx.SuspendCurrentQueueTime)
						continueConsume = false
					} else {
						commitOffset = pq.commit()
					}
