...
func (dc *defaultConsumer) start() error {
	...
	if dc.model == Clustering {
		...
		dc.storage = NewRemoteOffsetStore(dc.consumerGroup, dc.client, dc.client.Namesrvs())
	} else {
...
func (dc *defaultConsumer) lock(mq *primitive.MessageQueue) bool {
	brokerResult := dc.client.Namesrvs().FindBrokerAddressInSubscribe(mq.BrokerName, internal.MasterId, true)
...
func (dc *defaultConsumer) lockAll() {
	...
	for broker, mqs := range mqMapSet {
		...
		brokerResult := dc.client.Namesrvs().FindBrokerAddressInSubscribe(broker, internal.MasterId, true)
		if brokerResult == nil {
			continue
		}
		body := &lockBatchRequestBody{
...
func (dc *defaultConsumer) doUnlock(addr string, body *lockBatchRequestBody, oneway bool) {
	...
	if oneway {
		...
		if response != nil && response.Code != internal.ResSuccess {
			// TODO error
		}
	}
}
...
func (dc *defaultConsumer) findConsumerList(topic string) []string {
	brokerAddr := dc.client.Namesrvs().FindBrokerAddrByTopic(topic)
	if brokerAddr == "" {
		dc.client.Namesrvs().UpdateTopicRouteInfo(topic)
		brokerAddr = dc.client.Namesrvs().FindBrokerAddrByTopic(topic)
	}
...
func (dc *defaultConsumer) queryMaxOffset(mq *primitive.MessageQueue) (int64, error) {
	brokerAddr := dc.client.Namesrvs().FindBrokerAddrByName(mq.BrokerName)
	if brokerAddr == "" {
		dc.client.Namesrvs().UpdateTopicRouteInfo(mq.Topic)
		brokerAddr = dc.client.Namesrvs().FindBrokerAddrByName(mq.BrokerName)
	}
	if brokerAddr == "" {
...
func (dc *defaultConsumer) tryFindBroker(mq *primitive.MessageQueue) *internal.FindBrokerResult {
	result := dc.client.Namesrvs().FindBrokerAddressInSubscribe(mq.BrokerName, dc.recalculatePullFromWhichNode(mq), false)
	if result != nil {
		return result
	}
	dc.client.Namesrvs().UpdateTopicRouteInfo(mq.Topic)
	return dc.client.Namesrvs().FindBrokerAddressInSubscribe(mq.BrokerName, dc.recalculatePullFromWhichNode(mq), false)
}
