...
func (c *rmqClient) Namesrvs() Namesrvs {
	return c.namesrvs
}
...
func (c *rmqClient) UpdateTopicRouteInfo() {
	...
	for topic := range publishTopicSet {
		data, _, _ := c.namesrvs.UpdateTopicRouteInfo(topic)
		c.UpdatePublishInfo(topic, data, true)
	}
	...
	for topic := range subscribedTopicSet {
		data, _, _ := c.namesrvs.UpdateTopicRouteInfo(topic)
		c.updateSubscribeInfo(topic, data, true)
	}
}
...
func (c *rmqClient) updateSubscribeInfo(topic string, data *TopicRouteData, changed bool) {
	...
	rlog.Debug("updating topic subscribe info", map[string]interface{}{
		"topic":   topic,
		"route":   data,
		"changed": changed,
	})
