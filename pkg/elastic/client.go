...
func (c *Client) healthcheck(parentCtx context.Context, timeout time.Duration, force bool) {
	...
	for _, conn := range conns {
		...
		case <-ctx.Done(): // timeout
			c.errorf(context.Background(), "elastic: %s is dead", conn.URL())
			conn.MarkAsDead()
		case err := <-errc:
			if err != nil {
				c.errorf(context.Background(), "elastic: %s is dead", conn.URL())
			...
			if status >= 200 && status < 300 {
				...
				conn.MarkAsDead()
				c.errorf(context.Background(), "elastic: %s is dead [status=%d]", conn.URL(), status)
			}
		}
	}
}
...
func (c *Client) PerformRequest(ctx context.Context, opt PerformRequestOptions) (*Response, error) {
	...
	defer func() {
		...
		ctx = context.WithValue(ctx, EsLogKeyStartTime, start)
		ctx = context.WithValue(ctx, EsLogKeyEndTime, end)
		ctx = context.WithValue(ctx, EsLogKeyStatusCode, statusCode)
